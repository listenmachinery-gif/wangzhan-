  ...
  return (
    <main data-foot-operated-shearing-machine-page className="max-w-full overflow-x-clip bg-white text-[#111315]">
      ...
      <section data-section="materials" className="bg-[#F5F5F7] px-5 py-20 sm:px-8 lg:py-28">
        <div className="mx-auto max-w-7xl"><div className="grid gap-7 lg:grid-cols-[0.78fr_1.22fr] lg:items-end"><div><p className={sectionLabel}>{content.materials.eyebrow}</p><h2 className="mt-4 text-3xl font-semibold leading-tight sm:text-5xl">{content.materials.title}</h2></div><p className="border-l-4 border-[#76B900] bg-white p-5 text-sm leading-7 text-neutral-600">{content.materials.note}</p></div><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{content.materials.items.map((item,index)=><article key={item.title} className="group overflow-hidden bg-white shadow-[0_16px_44px_rgba(15,23,42,0.06)]"><div className="relative aspect-[8/5] overflow-hidden bg-neutral-200"><Image src={item.image} alt={item.alt} fill sizes="(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-[1.03]"/></div><div className="p-7"><span className="font-mono text-sm text-[#76B900]">0{index+1}</span><h3 className="mt-5 text-xl font-semibold">{item.title}</h3><p className="mt-4 text-sm leading-7 text-neutral-600">{item.text}</p></div></article>)}</div></div>
      </section>
      ...
      <section data-section="applications" className="px-5 py-20 sm:px-8 lg:py-28">
        <div className="mx-auto max-w-7xl"><p className={sectionLabel}>{content.applications.eyebrow}</p><h2 className="mt-4 text-3xl font-semibold leading-tight sm:text-5xl">{content.applications.title}</h2><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{content.applications.items.map((item)=><article key={item.title} className="group overflow-hidden border border-neutral-200 bg-white"><div className="relative aspect-[8/5] overflow-hidden bg-neutral-200"><Image src={item.image} alt={item.alt} fill sizes="(min-width: 1024px) 25vw, (min-width: 768px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-[1.03]"/></div><div className="p-6"><h3 className="text-lg font-semibold leading-snug">{item.title}</h3><p className="mt-3 text-sm leading-7 text-neutral-600">{item.text}</p></div></article>)}</div></div>
      </section>
